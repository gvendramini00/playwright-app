        ...
        try {
            return playwrightUtil.withPage(page -> {
                ...
                if (dto.phoneCountryLabel() != null) {
                    page.getByRole(AriaRole.COMBOBOX, new Page.GetByRoleOptions().setName(Pattern.compile("Portugal|País|Country", Pattern.CASE_INSENSITIVE))).click();
                    page.getByText(Pattern.compile(dto.phoneCountryLabel(), Pattern.CASE_INSENSITIVE)).click();
                }
                page.getByRole(AriaRole.TEXTBOX, new Page.GetByRoleOptions().setName(Pattern.compile("Telem(ó|o)vel\\*", Pattern.CASE_INSENSITIVE))).fill(dto.phone());

                String before = page.url();
                page.getByRole(AriaRole.LINK, new Page.GetByRoleOptions().setName(Pattern.compile("COMEÇAR|COMECAR", Pattern.CASE_INSENSITIVE)))
    ...
    public String runDuplicateNifTest() {
        ...
        try {
            return playwrightUtil.withPage(page -> {
                ...
                page.locator("#step2, #step1, body").getByText(Pattern.compile("CONTINUAR", Pattern.CASE_INSENSITIVE)).first().click();
