...
public class AliraTestServiceImpl implements AliraTestService {
    ...
    public String testCase003NavigateTabGamesRoomsTest() {
        try (Playwright playwright = Playwright.create(); Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(false))) {
            ...
            page.getByRole(AriaRole.LINK, new Page.GetByRoleOptions().setName(" Games")).click();
            page.getByRole(AriaRole.LINK, new Page.GetByRoleOptions().setName("Rooms").setExact(true)).click();

            String targetUrl = AliraVariables.BASE_URL.getValue() + "rooms.aml";
            page.waitForURL("**/" + "rooms.aml");

            if (page.url().equals(targetUrl)) {
                log.info("Success - Navigation to tab GAMES -> ROOMS");

                page.screenshot(new Page.ScreenshotOptions().setPath(Paths.get(AliraVariables.SCREENSHOT_PATH.getValue() + "testCase003.png")));

                return "✅ Rooms tab loaded correctly! URL: " + page.url();
            } else {
                return "❌ Navigation failed. Expected " + targetUrl + " but got " + page.url();
            }

        } catch (Exception e) {
            log.error("Navigation Error", e);
            return "❌ Error during navigation: " + e.getMessage();
        }
    }
    ...
    public String testCase004NavigateTabGamesProcessRoomsTest() {
        try (Playwright playwright = Playwright.create(); Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(false))) {
            ...
            String targetUrl = AliraVariables.BASE_URL.getValue() + "processRooms.aml";
            page.waitForURL("**/" + "processRooms.aml");

            if (page.url().equals(targetUrl)) {
                log.info("Success - Navigation to tab GAMES -> PROCESS ROOMS");

                page.screenshot(new Page.ScreenshotOptions().setPath(Paths.get(AliraVariables.SCREENSHOT_PATH.getValue() + "testCase004.png")));
    ...
    public String testCase005NavigateTabGamesLobbyRoomsTest() {
        try (Playwright playwright = Playwright.create(); Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(false))) {
            ...
            String targetUrl = AliraVariables.BASE_URL.getValue() + "lobby.aml";
            page.waitForURL("**/" + "lobby.aml");

            if (page.url().equals(targetUrl)) {
                log.info("Success - Navigation to tab GAMES -> LOBBY ROOMS");

                page.screenshot(new Page.ScreenshotOptions().setPath(Paths.get(AliraVariables.SCREENSHOT_PATH.getValue() + "testCase005.png")));
    ...
    public String testCase006NavigateTabGamesProvidersTest() {
        try (Playwright playwright = Playwright.create(); Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(false))) {
            ...
            String targetUrl = AliraVariables.BASE_URL.getValue() + "providers.aml";
            page.waitForURL("**/" + "providers.aml");

            if (page.url().equals(targetUrl)) {
                log.info("Success - Navigation to tab GAMES -> PROVIDERS");

                page.screenshot(new Page.ScreenshotOptions().setPath(Paths.get(AliraVariables.SCREENSHOT_PATH.getValue() + "testCase006.png")));
    ...
    public String testCase007NavigateTabGamesThemesTagsTest() {
        try (Playwright playwright = Playwright.create(); Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(false))) {
            ...
            String targetUrl = AliraVariables.BASE_URL.getValue() + "themesTags.aml";
            page.waitForURL("**/" + "themesTags.aml");

            if (page.url().equals(targetUrl)) {
                log.info("Success - Navigation to tab GAMES -> Themes & Tags");

                page.screenshot(new Page.ScreenshotOptions().setPath(Paths.get(AliraVariables.SCREENSHOT_PATH.getValue() + "testCase007.png")));
    ...
    public String testCase008NavigateTabGamesExchangeProfileTest() {
        try (Playwright playwright = Playwright.create(); Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(false))) {
            ...
            String targetUrl = AliraVariables.BASE_URL.getValue() + "exchangeProfile.aml";
            page.waitForURL("**/" + "exchangeProfile.aml");

            if (page.url().equals(targetUrl)) {
                log.info("Success - Navigation to tab GAMES -> Exchange Profile");

                page.screenshot(new Page.ScreenshotOptions().setPath(Paths.get(AliraVariables.SCREENSHOT_PATH.getValue() + "testCase008.png")));
    ...
    public String testCase009NavigateTabWebsiteCMSTest() {
        try (Playwright playwright = Playwright.create(); Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(false))) {
            ...
            String targetUrl = AliraVariables.BASE_URL.getValue() + "cms.aml#elf_l1_Lw";
            page.waitForURL("**/" + "cms.aml#elf_l1_Lw");

            if (page.url().equals(targetUrl)) {
                log.info("Success - Navigation to tab WEBSITE -> CMS");

                page.screenshot(new Page.ScreenshotOptions().setPath(Paths.get(AliraVariables.SCREENSHOT_PATH.getValue() + "testCase009.png")));
    ...
    public String testCase010NavigateTabWebsiteConfigurationCMSAccessTest() {
        try (Playwright playwright = Playwright.create(); Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(false))) {
            ...
            String targetUrl = AliraVariables.BASE_URL.getValue() + "CMSAccess.aml?id=845";
            page.waitForURL("**/" + "CMSAccess.aml?id=845");

            if (page.url().equals(targetUrl)) {
                log.info("Success - Navigation to tab WEBSITE -> Configuration -> CMS Access");

                page.screenshot(new Page.ScreenshotOptions().setPath(Paths.get(AliraVariables.SCREENSHOT_PATH.getValue() + "testCase010.png")));
