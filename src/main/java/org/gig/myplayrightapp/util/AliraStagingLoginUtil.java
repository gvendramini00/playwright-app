...
@Slf4j
@Component
public class AliraStagingLoginUtil {

    @Value("${alira.staging.username}")
    private String username;

    @Value("${alira.staging.password}")
    private String password;

    @Value("${alira.staging.login-url}")
    private String loginUrl;

    /**
     * Reusable login helper for Alira Staging Back Office.
     * @param page The current Playwright Page instance
     */
    public void login(Page page) {
        log.info("Utility: Logging in staging user {}", username);
        ...
        page.waitForSelector("text=" + AliraVariables.ALIRA_DASHBOARD.getValue(),
                new Page.WaitForSelectorOptions()
                        ...
                        .setTimeout(10000));
        log.info("Utility: Staging login successful.");
    }
}
